...
prepare_ensemble_data <- function(wp_name, ensemble_data, weather_params, file_path, start_time, end_time, toScale) {
  ...
  training_data[,"hour"] <- hour(training_data$time)
  training_data[,"b4"] <- cos(((2 * pi) / 24) * training_data$hour)
  training_data[,"b5"] <- sin(((2 * pi) / 24) * training_data$hour)
  training_data[,"b6"] <- cos(((4 * pi) / 24) * training_data$hour)
  training_data[,"b7"] <- sin(((4 * pi) / 24) * training_data$hour)
  ...
  for (j in 1:51) {
    ...
    train_data[,"speed2"] <- (ensemble_data[["speed"]][,ens_name])^2
